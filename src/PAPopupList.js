...
class PAPopupList extends React.Component {
	render() {
		...
		children = this.props.dataForPopupList.features.map((feature) => {
			...
			return <tr className={'wdpaPopupListItem'} key={feature.properties.wdpa_pid} onMouseEnter={this.props.showPAPopup.bind(this, feature)}><td>{(status!=='removed') ? feature.properties.wdpa_pid : ''}</td><td><Status status={status} iconOnly={true}/></td><td>{feature.properties.name} ({feature.properties.iucn_cat})</td></tr>;
		});
